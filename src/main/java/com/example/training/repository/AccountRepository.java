...
public interface AccountRepository extends JpaRepository<AccountInformation, Long> {
    ...
    @Modifying
    @Query(value = "UPDATE account_information SET LINKED_EMAIL_ADDRESS = ?2, PASSWORD = ?3 WHERE ID = ?1", nativeQuery = true)
    Integer updateById(long id, String newEmailAddress, String newPassword);

    @Modifying
    @Query(value = "DELETE FROM account_information WHERE id = ?1", nativeQuery = true)
